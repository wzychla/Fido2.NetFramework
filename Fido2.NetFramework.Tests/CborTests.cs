using Microsoft.VisualStudio.TestTools.UnitTesting;
using System;
using System.Linq;

namespace Fido2NetLib.Cbor.Tests
{
    [TestClass]
    public class CborTests
    {
        [TestMethod]
        public void CanRoundtripAttestationObject()
        {
            byte[] data = Convert.FromBase64String("");

            var @object = (CborMap)CborObject.Decode(data);

            Assert.AreEqual( "android-key", ( (CborTextString)@object["fmt"] ).Value );
            Assert.AreEqual( 158, ( (CborByteString)@object["authData"] ).Value.Length );

            var attStmt = (CborMap)@object["attStmt"];

            Assert.AreEqual( -7, ( (CborInteger)attStmt["alg"] ).Value );
            Assert.AreEqual( 1, ( (CborArray)attStmt["x5c"] ).Length );
            Assert.AreEqual( 70, ( (CborByteString)attStmt["sig"] ).Value.Length );

            CollectionAssert.AreEqual( data, @object.Encode() );
        }

        [TestMethod]
        public void CanReadObjectAndIgnoreRemainingData()
        {
            byte[] data = Convert.FromBase64String("pQECAzguIVggy6rAanAqDpv7VJqZVJWgUVG6jIQvURLOH9Ylf/+nyesiWCDb75V3vgcbm7P0M1pI55cc6EkopDxVV7RXKQfm/xNlNyAIoWd0ZXN0aW5n9Q==");

            var map = (CborMap)CborObject.Decode(data, out int read);

            Assert.AreEqual( 88, data.Length );
            Assert.AreEqual( 78, read );

            var keys = map.Keys.Select(k => ((CborInteger)k).Value);

            Assert.AreEqual( 2, ( (CborInteger)map[new CborInteger( 1 )] ).Value );
            Assert.AreEqual( -47, ( (CborInteger)map[new CborInteger( 3 )] ).Value );
            Assert.AreEqual( "y6rAanAqDpv7VJqZVJWgUVG6jIQvURLOH9Ylf/+nyes=", Convert.ToBase64String( ( (CborByteString)map[new CborInteger( -2 )] ).Value ) );
            Assert.AreEqual( "2++Vd74HG5uz9DNaSOeXHOhJKKQ8VVe0VykH5v8TZTc=", Convert.ToBase64String( ( (CborByteString)map[new CborInteger( -3 )] ).Value ) );
            Assert.AreEqual( 8, ( (CborInteger)map[new CborInteger( -1 )] ).Value );

            // Ensure we ignored the remaining data

            var encoded = map.Encode();

            Assert.AreEqual( "pQECAzguIVggy6rAanAqDpv7VJqZVJWgUVG6jIQvURLOH9Ylf/+nyesiWCDb75V3vgcbm7P0M1pI55cc6EkopDxVV7RXKQfm/xNlNyAI", Convert.ToBase64String( encoded ) );

            Assert.AreEqual( 78, encoded.Length );
        }
    }
}