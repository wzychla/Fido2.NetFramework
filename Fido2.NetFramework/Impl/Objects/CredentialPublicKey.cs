...
namespace Fido2NetLib.Objects
{
    ...
    public sealed class CredentialPublicKey
    {
            ...
            switch ( _type )
            {
                case COSE.KeyType.EC2:
                    using ( ECDsa ecdsa = CreateECDsa() )
                    {
                        ...
                        return ecdsa.VerifyData( data.ToArray(), ecsig, CryptoUtils.HashAlgFromCOSEAlg( _alg ) );
                    }

                case COSE.KeyType.RSA:
                    using ( RSA rsa = CreateRSA() )
                    {
                        return rsa.VerifyData( data.ToArray(), signature.ToArray(), CryptoUtils.HashAlgFromCOSEAlg( _alg ), Padding );
                    }
        ...
        public ECDsa CreateECDsa()
        {
            ...
            switch ( (_alg, crv) )
            {
                case var tuple when tuple._alg == COSE.Algorithm.ES256K && tuple.crv == COSE.EllipticCurve.P256K:
                    curve = ECCurve.CreateFromFriendlyName( "secP256k1" );
                    break;
                case var tuple when tuple._alg == COSE.Algorithm.ES256 && tuple.crv == COSE.EllipticCurve.P256:
                    curve = ECCurve.NamedCurves.nistP256;
                    break;
                case var tuple when tuple._alg == COSE.Algorithm.ES384 && tuple.crv == COSE.EllipticCurve.P384:
                    curve = ECCurve.NamedCurves.nistP384;
                    break;
                case var tuple when tuple._alg == COSE.Algorithm.ES512 && tuple.crv == COSE.EllipticCurve.P521:
