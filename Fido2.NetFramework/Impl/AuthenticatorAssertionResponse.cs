...
namespace Fido2NetLib
{
    ...
    public sealed class AuthenticatorAssertionResponse : AuthenticatorResponse
    {
            ...
            CancellationToken cancellationToken = default )
        {
            ...
            if ( !string.Equals( Type, "webauthn.get", StringComparison.InvariantCultureIgnoreCase ) )
            ...
            byte[] hashedRpId = CryptoUtils.HashData256(Encoding.UTF8.GetBytes(rpid ?? string.Empty));
            byte[] hash = CryptoUtils.HashData256(Raw.Response.ClientDataJson);
            ...
            if ( AttestationObject != null )
            {
                ...
                if ( metadataService?.ConformanceTesting() is true && metadataEntry is null && attType != AttestationType.None && !string.Equals( fmt, "fido-u2f", StringComparison.InvariantCultureIgnoreCase ) )
            ...
            byte[] hash )
        {
            ...
            if ( storedDevicePublicKeys.Count > 0 )
            {
                ...
                else if ( matchedDpkRecords.Count == 0 )
                {
                    ...
                    List<DevicePublicKeyAuthenticatorOutput> matchedDpkKeys = new List<DevicePublicKeyAuthenticatorOutput>();
