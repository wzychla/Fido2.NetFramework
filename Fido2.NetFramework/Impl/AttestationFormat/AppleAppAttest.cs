...
namespace Fido2NetLib
{
    ...
    internal sealed class AppleAppAttest : AttestationVerifier
    {
        ...
        {
            var appleExtension = exts.OfType<X509Extension>().FirstOrDefault(e => e.Oid?.Value is "1.2.840.113635.100.8.5");
        ...
        public static readonly X509Certificate2 AppleAppAttestRootCA = new X509Certificate2(Convert.FromBase64String(appleAppAttestationRootCA));
        ...
        public static readonly Guid devAaguid = new Guid("61707061-7474-6573-7464-6576656c6f70");

        // "...or appattest followed by seven 0x00 bytes if operating in the production environment"
        // 61707061-7474-6573-7400-000000000000
        public static readonly Guid prodAaguid = new Guid("61707061-7474-6573-7400-000000000000");
        ...
        public override (AttestationType, X509Certificate2[]) Verify( VerifyAttestationRequest request )
        {
            ...
            if ( !( request.X5c is CborArray &&
                    ((CborArray)request.X5c).Length == 2 &&
                    ( (CborArray)request.X5c )[0] is CborByteString &&
                    ( (CborArray)request.X5c )[1] is CborByteString &&
                    ((CborByteString)( (CborArray)request.X5c )[0]).Length > 0 &&
                    ( (CborByteString)( (CborArray)request.X5c )[1] ).Length > 0 
                    ) )                 
            {
            ...
            var x5cArray = (CborArray)request.X5c;
            var chain = new X509Chain();
            chain.ChainPolicy.RevocationMode = X509RevocationMode.NoCheck;
            chain.ChainPolicy.ExtraStore.Add( AppleAppAttestRootCA );
            //chain.ChainPolicy.TrustMode = X509ChainTrustMode.CustomRootTrust;

            X509Certificate2 intermediateCert = new X509Certificate2((byte[])x5cArray[1]);
            chain.ChainPolicy.ExtraStore.Add( intermediateCert );

            X509Certificate2 credCert = new X509Certificate2((byte[])x5cArray[0]);
            ...
            var credCertPKHash = CryptoUtils.HashData256( credCert.GetPublicKey() );
            ReadOnlySpan<byte> keyIdentifier = credCert.GetNameInfo(X509NameType.SimpleName, false).FromHexString();
            if ( !credCertPKHash.SequenceEqual( keyIdentifier.ToArray() ) )
            {
            ...
            var appIdHash = CryptoUtils.HashData256( appId );
