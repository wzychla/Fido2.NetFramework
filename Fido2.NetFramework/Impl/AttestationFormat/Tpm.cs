...
namespace Fido2NetLib
{
    ...
    internal sealed class Tpm : AttestationVerifier
    {
        ...
        public override (AttestationType, X509Certificate2[]) Verify( VerifyAttestationRequest request )
        {
            ...
            if ( request.AttStmt["pubArea"] is CborByteString && 
                 ((CborByteString)request.AttStmt["pubArea"]).Length > 0
                )
            {
                pubArea = new PubArea( ( (CborByteString)request.AttStmt["pubArea"] ).Value );
            }
            ...
            var certInfo = 
                request.AttStmt["certInfo"] is CborByteString &&
                ((CborByteString)request.AttStmt["certInfo"]).Length > 0
                ? new CertInfo(((CborByteString)request.AttStmt["certInfo"]).Value)
            ...
            if ( request.X5c is CborArray &&
                 ((CborArray)request.X5c).Length > 0
                )
            {
                ...
                {
                    if ( x5cArray[i] is CborByteString &&
                        ((CborByteString)x5cArray[i]).Length > 0
                        )
                    {
                        var x5cObject = (CborByteString)x5cArray[i];
        ...
        private static (string, string, string) SANFromAttnCertExts( X509ExtensionCollection exts )
        {
            ...
            foreach ( var extension in exts )
            {
                if ( extension.Oid?.Value == "2.5.29.17" ) // subject alternative name
                {
                    ...
                    if ( subjectAlternativeName.Sequence.FirstOrDefault( o => o.TagClass == TagClass.ContextSpecific && o.TagValue == 4 /*Octet-String */ ) is Asn1Element generalName )
                    {
    ...
    public sealed class CertInfo
    {
        ...
        public CertInfo( byte[] data )
        {
            ...
            if ( 0xff544347 != BinaryPrimitives.ReadUInt32BigEndian( Magic ) )
                throw new Fido2VerificationException( "Bad magic number " + Magic.ToHexString() );

            Type = AuthDataHelper.GetSizedByteArray( data, ref offset, 2 );
            if ( 0x8017 != BinaryPrimitives.ReadUInt16BigEndian( Type ) )
                throw new Fido2VerificationException( "Bad structure tag " + Type.ToHexString() );
        ...
        private static readonly Dictionary<TpmAlg, ushort> s_tpmAlgToDigestSizeMap = new Dictionary<TpmAlg, ushort> ()
    {
    ...
    public sealed class PubArea
    {
        ...
        public PubArea( byte[] data )
        {
            ...
            if ( tpmAlg is TpmAlg.TPM_ALG_RSA || tpmAlg is TpmAlg.TPM_ALG_ECC )
            {
