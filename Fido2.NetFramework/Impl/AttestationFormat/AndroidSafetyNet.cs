...
namespace Fido2NetLib
{
    ...
    internal sealed class AndroidSafetyNet : AttestationVerifier
    {
        ...
        public override (AttestationType, X509Certificate2[]) Verify( VerifyAttestationRequest request )
        {
            ...
            if ( !( request.AttStmt["response"] is CborByteString &&
                    ((CborByteString)request.AttStmt["response"]).Length > 0 )
                )
                throw new Fido2VerificationException( Fido2ErrorMessages.InvalidSafetyNetResponse );

            var responseByteString = (CborByteString)request.AttStmt["response"];
            ...
            try
            {
                jwtHeaderBytes = Base64Url.Decode( jwtComponents[0].ToCharArray() );
            }
            ...
            using ( var jwtHeaderJsonDoc = JsonDocument.Parse( jwtHeaderBytes ) )
            {
                ...
                {
                    if ( claim.Type == "nonce" && claim.ValueType == "http://www.w3.org/2001/XMLSchema#string" && claim.Value.Length != 0 )
                    {
                        nonce = claim.Value;
                    }
                    if ( claim.Type == "ctsProfileMatch" && claim.ValueType == "http://www.w3.org/2001/XMLSchema#boolean" )
                    {
                        ctsProfileMatch = bool.Parse( claim.Value );
                    }
                    if ( claim.Type == "timestampMs" && claim.ValueType == "http://www.w3.org/2001/XMLSchema#integer64" )
                    {
                        timestamp = DateTimeOffset.FromUnixTimeMilliseconds( 0 ).AddMilliseconds( double.Parse( claim.Value, CultureInfo.InvariantCulture ) );
                    }
                }
                ...
                var dataHash = CryptoUtils.HashData256( request.Data );

                if ( !dataHash.SequenceEqual( nonceHash ) )
                {
                    throw new Fido2VerificationException( Fido2ErrorCode.InvalidAttestation, $"SafetyNet response nonce / hash value mismatch, nonce {nonceHash.ToHexString()}, hash {dataHash.ToArray().ToHexString()}" );
                }
                ...
                if ( !string.Equals( subject, "attest.android.com", StringComparison.InvariantCultureIgnoreCase ) )
