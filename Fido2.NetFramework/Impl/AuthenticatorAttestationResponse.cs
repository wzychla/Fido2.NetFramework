...
namespace Fido2NetLib
{
    ...
    public sealed class AuthenticatorAttestationResponse : AuthenticatorResponse
    {
            ...
            CancellationToken cancellationToken = default )
        {
            ...
            byte[] clientDataHash = CryptoUtils.HashData256(Raw.Response.ClientDataJson);
            byte[] rpIdHash = CryptoUtils.HashData256(Encoding.UTF8.GetBytes(originalOptions.Rp.Id));
            ...
            if ( Raw.Extensions?.DevicePubKey != null )
            {
            ...
            if ( metadataService?.ConformanceTesting() is true && metadataEntry is null && attType != AttestationType.None && !string.Equals( AttestationObject.Fmt, "fido-u2f", StringComparison.InvariantCultureIgnoreCase ) )
            ...
            CancellationToken cancellationToken )
        {
            ...
            // while conformance testing, we must reject any authenticator that we cannot get metadata for
            if ( metadataService?.ConformanceTesting() is true && metadataEntry is null && attType != AttestationType.None && !string.Equals(devicePublicKeyAuthenticatorOutput.Fmt, "fido-u2f", StringComparison.InvariantCultureIgnoreCase ) )
                throw new Fido2VerificationException( Fido2ErrorCode.AaGuidNotFound, "AAGUID not found in MDS test metadata" );
