...
namespace Fido2NetLib
{
    ...
    public sealed class ConformanceMetadataRepository : IMetadataRepository
    {
        ...
        public ConformanceMetadataRepository( string origin )
        {
            _origin = origin;
        }
        ...
        public async Task<MetadataBLOBPayload> GetBLOBAsync( CancellationToken cancellationToken = default )
        {
            ...
            var content = new ByteArrayContent( Encoding.UTF8.GetBytes( JsonConvert.SerializeObject(req, FidoSerializerContext.Default.GetBLOBRequest)))
            {
            ...
            using ( var response = await _httpClient.PostAsync( _getEndpointsUrl, content, cancellationToken ) )
            {
                ...
                MDSGetEndpointResponse result = JsonConvert.DeserializeObject<MDSGetEndpointResponse>(responseString, FidoSerializerContext.Default.MDSGetEndpointResponse);
        ...
        public async Task<MetadataBLOBPayload> DeserializeAndValidateBlobAsync( string rawBLOBJwt, CancellationToken cancellationToken = default )
        {
            ...
            using ( var jsonDoc = JsonDocument.Parse( Base64Url.Decode( blobHeader.ToCharArray() ) ) )
            {
                ...
                if ( !certChainIsValid )
                {
                    ...
                    if ( rootCert.Thumbprint.Equals( certChain.ChainElements[certChain.ChainElements.Count-1].Certificate.Thumbprint, StringComparison.Ordinal ) &&
                        ...
                        certChain.ChainElements[certChain.ChainElements.Count - 1].ChainElementStatus.All( c => c.Status == X509ChainStatusFlags.UntrustedRoot ) )
                    {
                ...
                MetadataBLOBPayload blob = JsonConvert.DeserializeObject<MetadataBLOBPayload>(blobPayload, FidoModelSerializerContext.Default.MetadataBLOBPayload);
                blob.JwtAlg = blobAlg;
                return blob;
            }
        }
